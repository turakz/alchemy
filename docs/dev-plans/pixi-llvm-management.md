...
build = { cmd = "cmake --build build/debug", depends-on = ["configure"] }
test-unit = { cmd = "cmake --build build/debug --target unit_tests && cd build/debug && ctest --output-on-failure --verbose -L unit_tests", depends-on = ["build"] }
test-integration = { cmd = "cmake --build build/debug --target integration_tests && cd build/debug && ctest --output-on-failure --verbose -L integration_tests", depends-on = ["build"] }
```
