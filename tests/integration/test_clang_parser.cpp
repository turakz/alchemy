...
TEST_F(ParserDatabaseIntegrationTest, ParsesIarKeywordsWithIarDatabase)
{
  alchemy::testing::utils::createIarDatabase(buildDir, projectDir);

  // iar_compat.h contains __interrupt inside a __IAR_SYSTEMS_ICC__ guard.
  // the mock compiler emits __IAR_SYSTEMS_ICC__ so the guard is active.
  // if __interrupt is missing from the compat defines clang skips
  // the parse and IrqConfig is not found.
  const std::vector<std::filesystem::path> HeaderFiles = {projectDir / "inc" /
                                                          "iar_compat.h"};
  ...
  ASSERT_TRUE(parserResult.valid())
      << "Parser should create with IAR compat header: "
      << parserResult.error();
  ...
  ASSERT_TRUE(parseResult.valid())
      << "Parser should successfully parse IAR compat header: "
      << parseResult.error();

  ASSERT_EQ(parseResult.value().structs.size(), 1)
      << "IrqConfig must be found meaning clang parsed the file despite "
         "__interrupt in the header";

  EXPECT_EQ(parseResult.value().structs[0].structName, "IrqConfig");
}
