...
**High Priority**:
1. ~~**`buildReverseDepMap` parallelization**~~ — **DONE** (v4.1: 65s→19s on debug_fw, 3.4×)
...
### Wall-Clock Improvement (debug_fw, ~500 compile commands)

| Metric | v4.0 | v4.1 | Change |
|--------|-------|------|--------|
| **Wall-clock (native, debug_fw)** | ~65s | **~19s** | **-46s (-70.8%, 3.4×)** |
...
### Real-World Callgrind Profile (debug_fw, 315 source files)
...
| **v4.1 (dep graph parallelization + --debug)** | *(not re-run, wall-clock: 65s→19s on debug_fw)* | — | — |
...
- v4.1 optimization: buildReverseDepMap parallelization (65s→19s on debug_fw), --debug flag + level-gated logging (2026-03-19)
