...
[
  ...
  ASSERT_EQ(configResult.value().language, "c");
}

TEST_F(CompilationDatabaseFactoryTest,
       TestGccExtractQueryConfigDetectsCppLanguage)
{
  const std::string DbContent = R"(
[
  {
    "directory": "/tmp/test_project",
    "file": "/tmp/test_project/src/main.cpp",
    "command": "arm-none-eabi-g++ -Iinclude -DDEBUG -c src/main.cpp"
  }
])";
  writeFile(DbContent, buildDir / "compile_commands.json");
  ...
  ASSERT_TRUE(configResult.valid());
  ASSERT_EQ(configResult.value().compilerPath, "arm-none-eabi-g++");
  ASSERT_EQ(configResult.value().language, "c++");
}
