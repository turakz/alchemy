...
TEST_F(CompilationDatabaseFactoryTest,
       TestIarExtractQueryConfigCapturesSpaceSeparatedArchFlags)
{
  // space-separated arch flag form used in real IAR project databases
  const std::string DbContent = R"([
  {
    "directory": "/tmp/test",
    "file": "/tmp/test/main.c",
    "command": "iccarm --cpu Cortex-M3 --fpu None --dlib_config normal -c main.c"
  }])";
  writeFile(DbContent, buildDir / "compile_commands.json");
  ...
  EXPECT_THAT(result.value().archFlags, ::testing::Contains("--cpu"));
  EXPECT_THAT(result.value().archFlags, ::testing::Contains("Cortex-M3"));
  EXPECT_THAT(result.value().archFlags, ::testing::Contains("--fpu"));
  EXPECT_THAT(result.value().archFlags, ::testing::Contains("None"));
  EXPECT_THAT(result.value().archFlags, ::testing::Contains("--dlib_config"));
  EXPECT_THAT(result.value().archFlags, ::testing::Contains("normal"));
}
...
TEST_F(CompilationDatabaseFactoryTest,
       TestIarQuerySystemIncludesReturnsDirectories)
{
  // verifies that querySystemIncludes returns parent directories, not file
  // paths — -isystem expects directories, not individual header files.
  // note: the stub file's parent directory (e.g. /tmp) also appears in the
  // result because real iccarm emits a #line marker for the TU being
  // preprocessed. this is harmless — clang searches there and finds nothing.
  const std::filesystem::path mockCompilerPath{MOCK_ICCARM_PATH};

  alchemy::parser::libclang::adapters::IarQueryConfig query;
  query.compilerPath = mockCompilerPath.string();
  query.archFlags = {};
  ...
  {
    EXPECT_FALSE(std::filesystem::path(path).has_extension())
        << "expected directory, got file path: " << path;
  }

  EXPECT_THAT(result.value(), ::testing::Contains("/opt/iar/arm/inc/c"));
  EXPECT_THAT(result.value(),
              ::testing::Contains("/opt/iar/arm/inc/c/aarch32"));
  EXPECT_THAT(result.value(),
              ::testing::Contains("/opt/iar/arm/CMSIS/Core/Include"));
  EXPECT_THAT(result.value(),
              ::testing::Contains("/opt/iar/arm/CMSIS/DSP/Include"));
}
...
[
  ...
  ASSERT_THAT(translated.CommandLine, ::testing::Contains("-D__interrupt="));
  ...
  ASSERT_THAT(translated.CommandLine, ::testing::Contains("-U__GNUC__"));
}
