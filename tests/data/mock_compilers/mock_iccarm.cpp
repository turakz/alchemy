...
// handles: --preprocess=l (include query) and --predef_macros (defines query)
// syntax matches `iccarm --help` in v9.20.2
...
main(int argc, char* argv[])
{
  ...
  // handle --predef_macros <output_file>
  ...
  if (predefMacros && !outputFile.empty())
  {
    ...
    // compiler identification macros present in all IAR ARM builds
    ofs << "#define __IAR_SYSTEMS_ICC__ 1\n";
    ofs << "#define __ICCARM__ 1\n";
  ...
  {
    // real iccarm emits a line marker for the TU being preprocessed first
    if (!sourceFile.empty())
    {
      fmt::print("#line 1 \"{}\"\n", sourceFile);
    }
