    ...
    const clang::tooling::CompileCommand& gccCommand) const
{
  ...
  for (std::size_t idx = 1; idx < gccCommand.CommandLine.size(); ++idx)
  {
    ...
    // strip --driver-mode= injected by inferTargetAndDriverMode
    // -> alchemy handles flag translation directly, GCC driver mode is not
    // needed
  ...
  // GCC allows non-ASM statements in naked functions; clang hard-errors
  // (err_non_asm_stmt_in_naked_function, not suppressible via -W flags).
  // neutralize the attribute by redefining it to 'unused' via macro.
  translatedArgs.emplace_back("-Dnaked=unused");
    ...
    const clang::tooling::CompilationDatabase& db)
{
  ...
  alchemy::logger::debug("alchemy::parser::libclang::adapters::GccDbTranslator:"
                         ":extractQueryConfig: compiler={}\n",
                         query.compilerPath);
    ...
    std::string_view compilerOutput)
{
  ...
  std::istringstream ss{std::string(compilerOutput)};
  std::string line;
...
{
  // query both C and C++ system include paths from the compiler, merge and
  // deduplicate. a compilation database may contain a mix of C and C++ TUs
  // regardless of the compiler binary name, so both sets of paths are needed.
  // C++ paths are queried first so they appear earlier in the search order
  // -> this ensures #include_next in cxx std headers resolves correctly
  static constexpr std::array<const char*, 2> Languages = {"c++", "c"};
  std::vector<std::string> mergedIncludes;
  std::unordered_set<std::string> seen;

  for (const auto* lang : Languages)
  {
    const std::vector<std::string> Args = {
        "-E", "-Wp,-v", "-x", lang, "/dev/null"};
    ...
    if (outputResult.invalid())
    {
      alchemy::logger::debug(
          "alchemy::parser::libclang::adapters::GccDbTranslator::"
          "querySystemIncludes: {} query failed for {}: {}\n",
          lang,
          query.compilerPath,
          outputResult.error());
      continue;
    ...
    for (auto& inc : includes)
    {
      ...
      {
        mergedIncludes.emplace_back(std::move(inc));
      }
    }
  }

  if (mergedIncludes.empty())
  {
