    ...
    const clang::tooling::CompileCommand& iarCommand) const
{
  ...
  // hardcoded to ARM because alchemy only detects iccarm today
  // future: support other IAR compilers (iccavr, iccrx, iccrl78, etc.) by
  // detecting the target from the compiler binary name, version banner
  // ("IAR C/C++ Compiler ... for ARM"), or predefined macros
  // (__ICCARM__, __ICCAVR__, etc.) and mapping to the appropriate clang target
  // triple
    ...
    const clang::tooling::CompilationDatabase& db)
{
  ...
  for (std::size_t idx = 1; idx < cmd.CommandLine.size(); ++idx)
  {
    const auto& arg = cmd.CommandLine[idx];
        ...
        arg.starts_with("--endian") || arg.starts_with("--dlib_config"))
    {
      ...
      if ((arg.find('=') == std::string::npos) &&
          ((idx + 1) < cmd.CommandLine.size()) &&
          !cmd.CommandLine[idx + 1].starts_with("-"))
      {
        query.archFlags.push_back(cmd.CommandLine[++idx]);
      }
    }
  }
    ...
    const std::string& compilerOutput)
{
  ...
  while (std::getline(ss, line))
  {
    ...
    if (IsPath && startIdx != std::string::npos && endIdx != std::string::npos)
    {
      ...
      includes.insert(Include.parent_path());
    }
  }
    ...
    const alchemy::parser::libclang::adapters::IarQueryConfig& query)
{
  ...
  const alchemy::core::detail::TempFile StubFile{"alchemy_intrinsics_stub",
                                                 ".c"};
  ...
  // resolve symlinks: iccarm anchors its system include search to the
  // directory of the binary as invoked, so reaching it via a symlink (e.g.
  // /usr/local/bin/iccarm -> /opt/iarsystems/bxarm/arm/bin/iccarm) makes it
  // search the wrong tree and fail to find <intrinsics.h>
  std::error_code resolveErr;
  auto resolvedCompiler =
      std::filesystem::canonical(query.compilerPath, resolveErr);
  const std::string CompilerToInvoke =
      resolveErr ? query.compilerPath : resolvedCompiler.string();

  // execute compiler for system paths/toolchain paths
  auto outputResult =
      alchemy::parser::libclang::adapters::executeCompilerCommand(
          CompilerToInvoke, queryArgs, "IarDbTranslator::querySystemIncludes");
  if (outputResult.invalid())
  {
    return alchemy::core::Result<std::vector<std::string>>::failure(
        alchemy::core::Error::format(
            ...
            " if {} is a symlink, verify it resolves to a real iccarm binary "
            "(alchemy resolves symlinks before invoking iccarm because it "
            "anchors its include search to the binary's directory)",
            query.compilerPath,
            outputResult.error(),
            query.compilerPath));
  }
...
{
  // build query cmd, e.g.
  // `iccarm stub.c --cpu=<archFlag> --predef_macros predef_macros.txt`
  //
  // syntax matches what `iccarm --help` documents in v9.20.2:
  //   --predef_macros file|directory
  // older IAR docs describe a `--predef_macros[=n]` form that skips
  // compilation, but this binary rejects `=n` (treats the outfile as a second
  // source file). passing just the outfile compiles the stub and emits the
  // predef list — the stub is empty/trivial so the compile cost is negligible
  const alchemy::core::detail::TempFile StubFile{"alchemy_predef_macros_stub",
                                                 ".c"};
  const alchemy::core::detail::TempFile OutFile{"alchemy_predef_macros",
                                                ".txt"};
  // iccarm requires the source file to actually exist on disk, even when
  // we only care about the predef list. TempFile reserves a path; write
  // an empty translation unit so iccarm can open it
  std::ofstream{StubFile.path, std::ios::out}.close();
  std::vector<std::string> queryArgs = {
  ...
  auto outputResult =
      alchemy::parser::libclang::adapters::executeCompilerCommand(
          CompilerToInvoke, queryArgs, "IarDbTranslator::querySystemDefines");
  if (outputResult.invalid())
  {
    return alchemy::core::Result<std::vector<std::string>>::failure(
        alchemy::core::Error::format(
    ...
    getClangCompatibleDefines()
{
  return {// IAR intrinsic keywords — define as empty (no clang equivalent)
          ...
          "-Dstatic_assert(...)=",
          // prevent GCC compat path from activating alongside IAR path in CMSIS
          // headers
          "-U__GNUC__",
