...
alchemy::parser::StructExtractor::ExtractedTrailingComment
...
{
  ExtractedTrailingComment result;
...
alchemy::parser::StructExtractor::ExtractedSourceType
    ...
    llvm::StringRef buf, unsigned prevLineStart, std::size_t firstNonSpace)
{
  ...
  while (checkPos > 0)
  {
    const unsigned PrevEnd = skipBackwardNewline(buf, checkPos);
    if (PrevEnd == 0)
    {
      break;
    }

    const unsigned LineStart = findLineStart(buf, PrevEnd);
    ...
    llvm::StringRef buf, unsigned prevLineStart, std::size_t lastContent)
{
  ...
  while (searchPos >= 1)
  {
    if (buf[searchPos - 1] == '/' && buf[searchPos] == '*')
    {
      ...
      const unsigned LineStart = findLineStart(buf, OpenSlash);

      // skip indentation to find comment marker
      const unsigned CommentStart = skipForwardHorizWs(buf, LineStart);
...
alchemy::parser::StructExtractor::ExtractedPrecedingComment
...
{
  ExtractedPrecedingComment result;
  ...
  const unsigned PrevLineStart = findLineStart(FileBuf, PrevLineEnd);
    ...
    const clang::LangOptions& langOpts)
{
  ...
  for (const clang::FieldDecl* fieldDecl : structDecl->fields())
  {
    ...
    if (fieldResult.value().isBitField)
    {
      structDef.hasBitFields = true;
    }

    structDef.addField(std::move(fieldResult).value());
  }
